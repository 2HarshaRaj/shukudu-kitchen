...
`recipe.js` must load before `recipe-scaling.js` because `recipe-scaling.js` extends the scaling and formatting functions defined by the main recipe renderer. `wake-lock.js` loads after the recipe renderer so it can attach controls after Cooking Mode is rendered.
...
- Keep validation rules in `scripts/validate-recipes.js` and `scripts/validate-produce-weights.js`.
- Run validation after recipe data or validation config changes.
